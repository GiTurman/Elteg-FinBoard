...
const CashInflowAnalysisModal: React.FC<AnalysisModalProps> = ({ isOpen, onClose, data }) => {
    ...
    const categoryTotals = useMemo(() => {
        const totals = latestWeekData.reduce((acc, r) => { 
            const rate = r.rate || 1;
            acc[r.category] = (acc[r.category] || 0) + (r.actual || 0) * rate; 
            return acc; 
...
export const CashInflowView: React.FC<{ user: User }> = ({ user }) => {
  ...
  const currentWeekTotals = useMemo(() => {
    return currentWeekEntries.reduce((acc, curr) => {
      ...
      acc.budgeted += (Number(curr.budgeted) || 0) * rate;
      acc.actual += (Number(curr.actual) || 0) * rate;
      return acc;
    }, { budgeted: 0, actual: 0 });
  }, [currentWeekEntries]);
                ...
                <table className="w-full text-left text-sm">
                    <thead className="bg-gray-100 text-gray-600 font-bold uppercase text-xs">
                        <tr>
                            ...
                            <th className="px-4 py-3">კურსი</th>
                        ...
                        <tr key={entry.id}>
                            ...
                            <td className="px-2 py-1">
                                <select 
                                    ...
                                    onChange={e => {
                                        ...
                                        const newRate = newCurr === Currency.GEL ? 1 : (MOCK_RATES as any)[newCurr] || 1;
                                        handleUpdate(entry.id, 'currency', newCurr);
                                        handleUpdate(entry.id, 'rate', newRate);
                                    }} 
                                    className="w-full border-b p-1 bg-transparent focus:border-black outline-none"
                                >
                            ...
                            <td className="px-2 py-1">
                                <input 
                                    type="number" 
                                    value={entry.rate || 1} 
                                    onChange={e => handleUpdate(entry.id, 'rate', parseFloat(e.target.value))} 
                                    className="w-16 text-right border-b p-1 bg-transparent focus:border-black outline-none" 
                                    disabled={entry.currency === Currency.GEL}
                                />
                            </td>
                    ...
                    <tfoot className="bg-gray-800 text-white font-bold text-xs uppercase">
                        <tr>
                            <td colSpan={isTopLevel ? 5 : 4} className="px-4 py-3">ჯამი (კონსოლიდირებული ლარში)</td>
            ...
            <div className="mt-12 pt-8 border-t-2 border-dashed border-gray-200 space-y-3">
              ...
              {sortedWeekKeys.map(weekKey => {
                ...
                const totals = weekData.reduce((acc, r) => { 
                  const rate = r.rate || 1;
                  acc.budgeted += (r.budgeted || 0) * rate; 
                  acc.actual += (r.actual || 0) * rate; 
                  return acc; 
                }, { budgeted: 0, actual: 0 }); 
                ...
                return (
                  <div key={weekKey} className="border border-gray-200 rounded overflow-hidden bg-white shadow-sm">
                    ...
                    {isExpanded && (<div className="overflow-auto max-h-[60vh] relative"><table className="w-full text-left text-sm table-auto"><thead className="sticky top-0 z-10 bg-gray-100 text-gray-600 font-bold uppercase text-xs"><tr><th className="px-4 py-3 sticky left-0 bg-gray-100 z-10">დასახელება/კლიენტი</th><th className="px-4 py-3">თარიღი</th><th className="px-4 py-3">კატეგორია</th><th className="px-4 py-3">ვალუტა</th><th className="px-4 py-3">კურსი</th><th className="px-4 py-3 text-right">დაგეგმილი</th><th className="px-4 py-3 text-right">ფაქტიური</th><th className="px-4 py-3 text-right">გადახრა</th><th className="px-4 py-3">კომენტარი</th></tr></thead><tbody className="divide-y divide-gray-100 bg-white">{weekData.map(rec => { const variance = (rec.actual - rec.budgeted) * (rec.rate || 1); return (<tr key={rec.id}><td className="px-4 py-2 font-bold sticky left-0 bg-white">{rec.name}</td><td className="px-4 py-2 font-mono text-gray-500">{rec.date ? new Date(rec.date).toLocaleDateString('ka-GE') : '-'}</td><td className="px-4 py-2 text-xs font-bold">{rec.category}</td><td className="px-4 py-2 text-xs">{rec.currency || 'GEL'}</td><td className="px-4 py-2 text-xs font-mono">{rec.rate || 1}</td><td className="px-4 py-2 text-right font-mono text-gray-500">{formatNumber(rec.budgeted)}</td><td className="px-4 py-2 text-right font-mono text-green-700">{formatNumber(rec.actual)}</td><td className={`px-4 py-2 text-right font-mono font-bold ${variance >= 0 ? 'text-green-600' : 'text-red-600'}`}>{formatNumber(variance)}</td><td className="px-4 py-2 text-xs italic">{rec.comment}</td></tr>);})}</tbody><tfoot className="sticky bottom-0 z-10 bg-gray-800 text-white font-bold text-xs uppercase"><tr><td className="px-4 py-3 sticky left-0 bg-gray-800 z-10">ჯამი (კონსოლიდირებული ლარში)</td><td colSpan={4}></td><td className="px-4 py-3 text-right font-mono">{formatNumber(totals.budgeted)} ₾</td><td className="px-4 py-3 text-right font-mono">{formatNumber(totals.actual)} ₾</td><td className={`px-4 py-3 text-right font-mono text-base ${totalVariance >=0 ? 'text-green-400' : 'text-red-400'}`}>{formatNumber(totalVariance)} ₾</td><td></td></tr></tfoot></table></div>)}
                  </div>
                );
              })}
            </div>
