...
export const Layout: React.FC<LayoutProps> = ({ 
  ...
  const [isSubAccountingOpen, setIsSubAccountingOpen] = useState(false);
  ...
  const isSubAccountingActive = ['sub-accounting-proforma', 'sub-accounting-generated'].includes(activeTab);
  ...
  useEffect(() => {
    ...
    if (isSubAccountingActive) {
        setIsSubAccountingOpen(true);
    }
  }, [activeTab]);
      ...
      <aside className="hidden md:flex flex-col bg-white border-r border-gray-200 h-screen sticky top-0 z-20 overflow-y-auto w-64">
        ...
        <nav className="flex-1 py-4 space-y-0.5">
          ...
          {isSubAccountant && (
            <div className="pt-2">
              <button
                onClick={() => setIsSubAccountingOpen(!isSubAccountingOpen)}
                className={`flex items-center justify-between gap-3 px-4 py-2 w-full text-left border-l-2 transition-all duration-200 ${
                  isSubAccountingActive
                    ? 'border-black text-black font-bold bg-gray-50'
                    : 'border-transparent text-gray-500 hover:text-black hover:bg-gray-50'
                }`}
              >
                <div className="flex items-center gap-3">
                    <Calculator size={18} strokeWidth={isSubAccountingActive ? 2.5 : 2} />
                    {isSidebarExpanded && <span>{translations['sub-accounting'][language]}</span>}
                </div>
                {isSidebarExpanded && (isSubAccountingOpen ? <ChevronDown size={16} /> : <ChevronRight size={16} />)}
              </button>
              ...
              {isSubAccountingOpen && (
                <div className="pl-5 pt-1 space-y-0.5 animate-in fade-in duration-300">
                  <NavItem
                    icon={FileText}
                    label="პროფორმა ინვოისები"
                    active={activeTab === 'sub-accounting-proforma'}
                    onClick={() => setActiveTab('sub-accounting-proforma')}
                    isExpanded={isSidebarExpanded}
                  />
                  <NavItem
                    icon={FileCheck}
                    label="გენერირებული ინვოისები"
                    active={activeTab === 'sub-accounting-generated'}
                    onClick={() => setActiveTab('sub-accounting-generated')}
                    isExpanded={isSidebarExpanded}
                  />
                </div>
              )}
