      ...
      <div className="bg-gray-800 text-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <NumpadInput placeholder="ღირებულება" value={formData.value || ''} onChange={val => handleChange('value', val)} className="px-3 py-2 text-sm bg-gray-700 border border-gray-600 rounded focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none"/>
            ...
            <NumpadInput placeholder="რაოდენობა" value={formData.quantity || ''} onChange={val => handleChange('quantity', val)} min={1} className="px-3 py-2 text-sm bg-gray-700 border border-gray-600 rounded focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none"/>
            <NumpadInput placeholder="სართული/გაჩერება" value={formData.floorsOrStops || ''} onChange={val => handleChange('floorsOrStops', val)} min={1} className="px-3 py-2 text-sm bg-gray-700 border border-gray-600 rounded focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none"/>
            <NumpadInput placeholder="ვადა (კვირა)" value={formData.durationInWeeks || ''} onChange={val => handleChange('durationInWeeks', val)} min={1} className="px-3 py-2 text-sm bg-gray-700 border border-gray-600 rounded focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none"/>
            <NumpadInput placeholder="სულ მიღებული თანხა" value={formData.totalReceived || ''} onChange={val => handleChange('totalReceived', val)} className="px-3 py-2 text-sm bg-gray-700 border border-gray-600 rounded focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none"/>
          </div>
      ...
      <div className="bg-gray-800 text-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <select value={formData.currency} onChange={e => handleChange('currency', e.target.value as Currency)} className="px-3 py-2 text-sm bg-gray-700 border border-gray-600 rounded focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none"><option value="GEL">GEL</option><option value="USD">USD</option><option value="EUR">EUR</option></select>
            <NumpadInput placeholder="რაოდენობა" value={formData.quantity || ''} onChange={val => handleChange('quantity', val)} min={1} className="px-3 py-2 text-sm bg-gray-700 border border-gray-600 rounded focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none"/>
            <NumpadInput placeholder="სართული/გაჩერება" value={formData.floorsOrStops || ''} onChange={val => handleChange('floorsOrStops', val)} min={0} className="px-3 py-2 text-sm bg-gray-700 border border-gray-600 rounded focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none"/>
            <NumpadInput placeholder="მიწოდების ვადა (კვირა)" value={formData.durationInWeeks || ''} onChange={val => handleChange('durationInWeeks', val)} min={1} className="px-3 py-2 text-sm bg-gray-700 border border-gray-600 rounded focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none"/>
            <NumpadInput placeholder="სულ მიღებული თანხა" value={formData.totalReceived || ''} onChange={val => handleChange('totalReceived', val)} className="px-3 py-2 text-sm bg-gray-700 border border-gray-600 rounded focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none"/>
          </div>
          <div className="space-y-2 pt-4 border-t border-gray-700"><label className="font-bold text-sm text-gray-300">ფასის ანალიზი</label><div className="grid grid-cols-1 md:grid-cols-4 gap-4">{[{l:'ერთეულის ღირ.',v:analysis.unitPrice},{l:'სართულის ღირ.',v:analysis.stopPrice},{l:'კვირის ღირ.',v:analysis.weeklyPrice},{l:'თვის ღირ.',v:analysis.monthlyPrice}].map(item=>(<div key={item.l} className="bg-gray-700/50 p-3 rounded-md text-center border border-gray-600"><div className="text-xs text-gray-400 uppercase">{item.l}</div><div className="font-bold text-lg font-mono">{formatNumber(item.v)}</div></div>))}</div></div>
          <div className="space-y-2 pt-4 border-t border-gray-700"><label className="font-bold text-sm text-gray-300">გადახდის გრაფიკი (ტრანშები)</label>{formData.tranches.map((tranche, index) => (<div key={tranche.id} className="flex items-center gap-2 bg-gray-900/50 p-2 rounded-md"><NumpadInput placeholder="%" value={tranche.percentage || ''} onChange={val => handleTrancheChange(index, 'percentage', val)} className="w-20 px-3 py-2 text-sm bg-gray-700 border border-gray-600 rounded" max={100} min={0}/><select value={tranche.month} onChange={e => handleTrancheChange(index, 'month', parseInt(e.target.value))} className="flex-1 px-3 py-2 text-sm bg-gray-700 border border-gray-600 rounded">{MONTH_NAMES_GE.map((m, i) => <option key={i} value={i}>{m}</option>)}</select><select value={tranche.year} onChange={e => handleTrancheChange(index, 'year', parseInt(e.target.value))} className="px-3 py-2 text-sm bg-gray-700 border border-gray-600 rounded"><option>2025</option><option>2026</option><option>2027</option></select><button type="button" onClick={() => removeTranche(index)} disabled={formData.tranches.length === 1} className="p-2 text-red-400 disabled:opacity-50 hover:bg-gray-700 rounded"><Trash2 size={16}/></button></div>))}<div className="flex items-center justify-between"><button type="button" onClick={addTranche} disabled={formData.tranches.length >= 4} className="text-xs font-bold text-blue-400 disabled:opacity-50 hover:underline">+ ტრანშის დამატება</button><div className={`text-xs font-bold ${Math.round(totalPercentage) !== 100 ? 'text-red-400' : 'text-green-400'}`}>სულ: {totalPercentage}%</div></div></div>
